...
module 'TableView', ->
    class @TableContainer extends Backbone.View
        ...
        maybe_refetch_data: =>
            # TODO: need more complicated logic here to determine if
            # an info error had previously occurred, and whether we
            # should fetch info. If index_unavailable changed, then we
            # may just want to fetch info now
            if @model.get('info_unavailable') and not @model.get('index_unavailable')
                console.log "would've refetched here"
        ...
        clear_cached_error_messages: (type) =>
            if @current_errors[type].length > 0
                console.log "Clearing #{type} errors"
                @current_errors[type] = []
